...
def redraw(db,best_param,gnuplot,options,tofile=False):
	...
	best_log2c,best_log2g,best_mse = best_param
	...
	gnuplot.write("set label \"Best log2(C) = {0}  log2(gamma) = {1}  mse = {2}\" \
				  at screen 0.5,0.85 center\n". \
				  format(best_log2c, best_log2g, best_mse).encode())
...
def find_parameters(dataset_pathname, options=''):
	...
	for line in jobs:
		...
		if gnuplot and options.grid_with_c and options.grid_with_g:
			redraw(db,[best_c, best_g, best_mse],gnuplot,options)
			redraw(db,[best_c, best_g, best_mse],gnuplot,options,True)
